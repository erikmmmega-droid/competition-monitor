...
- Будь конкретен и практичен в рекомендациях"""
        ...
        try:
            ...
            logger.debug(f"  Использовано токенов: {getattr(response.usage, 'total_tokens', 'N/A')}")
...
Формат ответа (строго JSON):
{
    ...
    "offer_score": 7,
    "trust_score": 6,
    "sales_argument_score": 5,
    "automation_depth": 4,
    "market_positioning": "Краткое описание позиционирования на рынке",
    "recommendations": ["рекомендация 1", "рекомендация 2", ...]
}

Важно:
 - Все числовые оценки (`*_score`, `automation_depth`) от 0 до 10
- Каждый массив должен содержать 3-5 пунктов
- Пиши на русском языке
 - - Оценивай: коммерческую привлекательность, элементы доверия, аргументацию продажи, возможности автоматизации продаж"""
        ...
        try:
            ...
            result = ImageAnalysis(
                ...
                offer_score=data.get("offer_score", 5),
                trust_score=data.get("trust_score", 5),
                sales_argument_score=data.get("sales_argument_score", 5),
                automation_depth=data.get("automation_depth", 5),
                market_positioning=data.get("market_positioning", ""),
                recommendations=data.get("recommendations", [])
            )

            logger.info(f"  Результат: offer {result.offer_score}/10, trust {result.trust_score}/10, sales_arg {result.sales_argument_score}/10, automation {result.automation_depth}/10")
            logger.info(f"  Позиционирование: {result.market_positioning}")
